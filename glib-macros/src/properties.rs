    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            ...
            if ident == "wrapper_type" {
                ...
                ext_trait = Some(input.parse::<syn::Path>()?);
            }
            if input.peek(Token![,]) {
                input.parse::<Token![,]>()?;
            }
...
pub fn impl_derive_props(input: PropsMacroInput) -> TokenStream {
    ...
    let rust_interface = if let Some(ext_trait) = ext_trait {
        ...
        quote! {
            #trait_def
            impl #ext_trait for #wrapper_type {
                #(#impls)*
            }
        }
    } else {
